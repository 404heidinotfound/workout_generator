...
if not os.path.exists(filename): # Prüfen, ob Datei existiert
...
cardio = ["Bike", "Row", "Ski Erg"] # bei diesen exercices muss meter oder calorie sein
...
formats = {
    ...
    "gymnastics": ["EMOM", "For Quality"],
    "classic": ["AMRAP", "EMOM", "For Time", "Rounds For Time"],
    "oly": ["5x5", "3x3", "Find 1RM", "EMOM 15min", "Every 30s for 15min"]
}
...
def filter_exercises_by_category_no_benchmarks(exercises, category):
        filtered = []
...
classic_exercises = filter_exercises_by_category(exercises, "classic") # Testen
print(f"Classic Übungen: {len(classic_exercises)}")

gym_exercises = filter_exercises_by_category(exercises, "gymnastics") # Testen
print(f"Gymnastics Übungen: {len(gym_exercises)}")

condi_exercises = filter_exercises_by_category(exercises, "conditioning") # Testen
print(f"Conditioning Übungen: {len(condi_exercises)}")

oly_exercises = filter_exercises_by_category(exercises, "oly") # Testen
print(f"Oly Übungen: {len(oly_exercises)}")

benchmarks = filter_benchmark_exercises(exercises)  # Testen
print(f"Benchmarks: {len(benchmarks)}")

def generate_exercise_specs(exercise_name, exercise_info, category, level): 
    #reps, gewichte, units für die Übungen generieren
    result = {
    ...
    # 1. unit bestimmen (reps oder meters/calories)
...
wod_rules = {
    ...
    "classic": {
        ...
        "duration": (10, 25),
        "exercises": (2, 5),
        "benchmark_chance": 0.2  # 10% Chance, ein Benchmark zu ziehen
    },
    "oly": {
...
def generate_wod(exercises, category):
    ...
    # Sonderfall Classic: evtl. Benchmark
    if category == "classic":
        # 10% Chance Benchmark, nur wenn Benchmarks existieren
    ...
    # normale Übungsauswahl
    filtered = filter_exercises_by_category_no_benchmarks(exercises, category)
    if not filtered:
